...
.factory('utils', ['$rootScope', '$location', '$http', '$modal', '$timeout',
	function($rootScope, $location, $http, $modal, $timeout) {
		...
		return {
			...
			popup: function(msg, type, close) {
				var tp;
				angular.element('.alert[name="bfd-alert"]').remove();
				switch (type) {
					case 'success':
						close == 'close' ?
							tp = angular.element('<div class="alert alert-success" name="bfd-alert" role="alert"><i class="fa fa-check-circle" style="color: #3c763d;"></i>' + msg + '<a style="margin-left: 10px;font-size: 14px;"><span class="fa fa-close"></span></a></div>') : tp = angular.element('<div class="alert alert-success" role="alert"><i class="fa fa-check-circle" style="color: #3c763d;"></i>' + msg + '</div>');
						break;
					case 'warning':
						close == 'close' ?
							tp = angular.element('<div class="alert alert-warning" name="bfd-alert" role="alert"><i class="fa fa-info-circle" style="color: #FF5400;"></i>' + msg + '<a style="margin-left: 10px;font-size: 14px;"><span class="fa fa-close"></span></a></div>') : tp = angular.element('<div class="alert alert-warning" role="alert"><i class="fa fa-info-circle" style="color: #FF5400;"></i>' + msg + '</div>');
						break;
					case 'error':
						close == 'close' ?
							tp = angular.element('<div class="alert alert-danger" name="bfd-alert" role="alert"><i class="fa fa-times-circle" style="color: #ff7143;"></i>' + msg + '<a style="margin-left: 10px;font-size: 14px;"><span class="fa fa-close"></span></a></div>') : tp = angular.element('<div class="alert alert-danger" role="alert"><i class="fa fa-times-circle" style="color: #ff7143;"></i>' + msg + '</div>');
						break;
					default:
						break;
				}
