...
<div class="input-group">\
	...
	.controller('imglibWinCtrl', ['$scope', '$modalInstance', '$http', '$modal', 'imgConfig', 'utils', function($scope, $modalInstance, $http, $modal, imgConfig, utils) {
		...
		$scope.imgConfig = {
			...
			uploadStart: function(uploader, file) { //当文件开始上传时
				...
				uploader.option('formData', {
					'zdyName': $scope.fileQueued[id].name
				});
			},
		...
		//加载图片库列表
		$http.get('http://sunny586.github.io/app/pccms/file/list')
		...
		$scope.findPic = function(imgParam) {
			$http({
					method: 'GET',
					url: 'http://sunny586.github.io/app/pccms/file/list?fileName=' + imgParam,
				})
...
.directive('imgLib', ['$modal', '$parse', function($modal, $parse) {
			...
			link: function(scope, element, attrs, ngModel) {
				...
				element.bind('click', function(e) {
					...
					var modalInstance = $modal.open({
						backdrop: 'static',
						templateUrl: 'http://sunny586.github.io/app/partials/imglibWin.html',
	...
	.controller('taglibWinCtrl', ['$scope', '$modalInstance', '$http', '$modal', 'utils', '$animate', 'result',
		function($scope, $modalInstance, $http, $modal, utils, $animate, result) {
			...
			//初始化查询标签列表。    
			function initTags() {
				$http.get('http://sunny586.github.io/app/demo/json/tag.json')
					.success(function(data, status, headers, config) {
	...
	return {
		...
		link: function(scope, element, attrs, ngModel) {
			...
			element.bind('click', function(e) {
				...
				var modalInstance = $modal.open({
					backdrop: 'static',
					templateUrl: 'http://sunny586.github.io/app/partials/taglibWin.html',
